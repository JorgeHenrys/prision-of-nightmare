...
public class LevelGeneration : MonoBehaviour
{
    ...
    private void Move()
    {
        ...
        else if (direction == 5)
        {
            ...
            {
                if(gameController.totalEnemies <= 4)
                {
                    GameObject tmp  = GameObject.FindWithTag("Room");
                    while (tmp != null) {
                        Destroy(tmp);
                        tmp  = GameObject.FindWithTag("Room");
                    }

                    // hasDestroyed = true;
                    // Collider2D roomDetection = Physics2D.OverlapCircle(centerMap.position, 100, room);
                    // roomDetection.GetComponent<RoomType>().RoomDestruction();
